...
{
    wifi_util_error_print(WIFI_DB,"%s:%d: KondammaEntry \n",__func__, __LINE__);
    struct schema_Wifi_Security_Config  *pcfg;
    json_t *where;
    wifi_db_t *g_wifidb;
    g_wifidb = (wifi_db_t*) get_wifidb_obj();
...
#endif /* CONFIG_IEEE80211BE */
    free(pcfg);
    wifi_util_error_print(WIFI_DB,"%s:%d: KondammaExit \n",__func__, __LINE__);
    return sec_mode_old;
}
...
int wifidb_update_wifi_security_config(char *vap_name, wifi_vap_security_t *sec)
{
    ...
    wifidb_get_wifi_security_config_old_mode(vap_name, vap_index);
